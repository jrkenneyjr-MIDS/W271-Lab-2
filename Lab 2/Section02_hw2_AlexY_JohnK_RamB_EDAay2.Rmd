...
```{r histograms}
lcat<-c("FY16GivingCat","FY14GivingCat","FY15GivingCat","FY13GivingCat","FY12GivingCat")
dt[,lcat,with=FALSE] %>%
  na.omit()%>%
  gather()%>%
  ggplot(aes(value))+
...
```{r}
prop.table(table(dt$Marital.Status))
```
...
```{r}
prop.table(table(dt$NextDegCat))
```
...
ggScatterJitter<-function(yearcat1,yearcat2){
  yeargive1<-paste("FY",toString(yearcat1),"GivingCat",sep='')
  yeargive2<-paste("FY",toString(yearcat2),"GivingCat",sep='')
  #print(yeargive)
...
```{r}
GenXtab(dframe=dt,x1=dt$AttendenceEvent,x2=dt$FY16GivingCat,nlist=c("Attendence","FY16Cat"))
```
...
ggplot(dt,aes(factor(Marital.Status),as.numeric(FY16GivingCat))) +
  ...
  scale_y_continuous(name="Giving Category",breaks=1:5,labels=c("[0,1)","[1,100)","[100,250)",
                                                                "[250,500)","[500,2e+05)")) +
  theme(plot.title = element_text(lineheight=1, face="bold"))
GenXtab(dframe=dt,x1=dt$Marital.Status,x2=dt$FY16GivingCat,nlist=c("Marital","FY16Cat"))
```
...
```{r}
GenXtab(dframe=dt,x1=dt$ClassYearCat,x2=dt$FY16GivingCat,nlist=c("Class","FY16Cat"))
```
...
```{r}
GenXtab(dframe=dt,x1=dt$Gender,x2=dt$ClassYearCat,nlist=c("Gender","Class"))
```
...
```{r}
GenXtab(dframe=dt,x1=dt$FY15GivingCat,x2=dt$FY16GivingCat,nlist=c("FY16","FY15"))
```
