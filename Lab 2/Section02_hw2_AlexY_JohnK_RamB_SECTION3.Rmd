...
dt$MeanGiving = (dt$FY12Giving + dt$FY13Giving + dt$FY14Giving + dt$FY15Giving)/4
dt$MeanGivingCat<-cut(dt$MeanGiving,c(0,1,100,250,500,200000),right=FALSE)
describe(dt$MeanGivingCat)
#Changing the yearly categorical variables into ordered factors
l1<-c("FY16GivingCat","FY15GivingCat","FY14GivingCat","FY13GivingCat","FY12GivingCat", "MeanGivingCat")
...
PredAcc = function(data_test, mod){
  ...
  if (class(mod)[1] == "glm"){
    ...
    test.pred = predict(mod, newdata = data_test, type = 'class')  #Category of highest probability
  }
...
```
```{r}
describe(dt_train$MeanGivingCat)

library(ordinal)

ord.mod1a = clm(FY16GivingCat~FY15GivingCat+FY14GivingCat+FY13GivingCat+FY12GivingCat, data = dt_train, model = TRUE)
summary(ord.mod1a)
Anova(ord.mod1a, test="Chisq")

CMTr = PredAcc(dt_train, mod=ord.mod1a)
CMTe = PredAcc(dt_test, mod=ord.mod1a)
ac_table_4 =  Accuracy(CMTr, CMTe, "ord.mod1a")
ac_table_4

#Average of All Years By Itself
ord.mod1b = clm(FY16GivingCat~MeanGivingCat, data = dt_train, model = TRUE)
summary(ord.mod1b)
Anova(ord.mod1b, test="Chisq")

CMTr = PredAcc(dt_train, mod=ord.mod1b)
CMTe = PredAcc(dt_test, mod=ord.mod1b)
ac_table_4 = rbind(ac_table_4, Accuracy(CMTr, CMTe, "ord.mod1b"))
ac_table_4

#FY15 By Itself
ord.mod1c = clm(FY16GivingCat~FY15GivingCat, data = dt_train, model = TRUE)
summary(ord.mod1c)
Anova(ord.mod1b, test="Chisq")

CMTr = PredAcc(dt_train, mod=ord.mod1c)
CMTe = PredAcc(dt_test, mod=ord.mod1c)
ac_table_4 = rbind(ac_table_4, Accuracy(CMTr, CMTe, "ord.mod1c"))
ac_table_4

head(dt_test)
#Kitchen Sink
ord.mod1d = clm(FY16GivingCat~FY15GivingCat+FY14GivingCat+FY13GivingCat+FY12GivingCat+MeanGivingCat+NextDegCat+ClassYearCat+MajorCat+MaritalStatusCat+YearsGiven, data = dt_train, model = TRUE)

ord.mod1d = clm(FY16GivingCat~MeanGivingCat+NextDegCat+ClassYearCat+MajorCat+MaritalStatusCat+YearsGiven, data = dt_train, model = TRUE)
ord.mod1d = multinom(FY16GivingCat~MeanGivingCat+NextDegCat+ClassYearCat+MajorCat+MaritalStatusCat+YearsGiven, data = dt_train, model = TRUE)
ord.mod1d = multinom(FY16GivingCat~FY15GivingCat+FY14GivingCat+FY13GivingCat+FY12GivingCat, data = dt_train, model = TRUE)
ord.mod1d = multinom(FY16GivingCat~FY15GivingCat+FY14GivingCat, data = dt_train, model = TRUE)

summary(ord.mod1d)
Anova(ord.mod1d, test="Chisq")

CMTr = PredAcc(dt_train, mod=ord.mod1d)
CMTe = PredAcc(dt_test, mod=ord.mod1d)
ac_table_4 =  rbind(ac_table_4,Accuracy(CMTr, CMTe, "ord.mod1d"))
ac_table_4
```
